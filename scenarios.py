...
def update_tech_evo(pv_lr=COPPER_LR_PV, wind_lr=COPPER_LR_WIND, write_csv=False):
    evol_fn = "copper/scenarios/CER_scenario/technology_evolution.csv"
    tech_evo = pd.read_csv(evol_fn, index_col=0)
    pds = [int(col) for col in tech_evo.columns]
    tech_evo.loc["solar.base", :] = lr_based_cost_factors(
        PV_CAPACITY_PROJECTION, pv_lr
    ).loc[pds].values
    tech_evo.loc["wind_ons.base", :] = lr_based_cost_factors(
        WIND_CAPACITY_PROJECTION, wind_lr
    ).loc[pds].values
    if write_csv:
